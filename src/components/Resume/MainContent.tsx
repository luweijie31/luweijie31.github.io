...
function isLocalizedStringArray(
  value: typeof resumeConfig.personal.summary,
): value is Record<string, string[]> {
  return !!value && Object.values(value).every(Array.isArray)
}
...
export function MainContent() {
  ...
  const profileSummary = personal.summary && isLocalizedStringArray(personal.summary)
    ? resolveArray(personal.summary)
    : null
  const profileParagraph = personal.summary && !isLocalizedStringArray(personal.summary)
  ...
  return (
    <div className="md:w-[62%] p-8">
      ...
      {/* Profile summary */}
      {personal.summary && labels.sections.profile && (
        <div className="mb-8">
          ...
          <div className="rounded-2xl border border-resume-primary/15 bg-gradient-to-br from-resume-primary/6 to-transparent px-5 py-4 shadow-sm">
            {profileParagraph && (
              <p className="text-sm leading-7 text-resume-text-secondary">
                {profileParagraph}
              </p>
            )}
            {profileSummary && (
              <ul className="space-y-2 text-sm leading-7 text-resume-text-secondary list-disc pl-5">
          ...
          <div className="space-y-1">
            {projects.map((project) => (
              <ProjectItem
                ...
                highlights={project.highlights ? resolveArray(project.highlights) : undefined}
                techs={project.techs}
                url={project.url}
